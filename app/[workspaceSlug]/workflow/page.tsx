...
import { GitFork, ArrowLeft, RefreshCw, ZoomIn, ZoomOut, Maximize } from 'lucide-react';
...
export default function WorkflowBuilder() {
  ...
  const getWorkflowElements = () => {
    ...
    statuses.forEach((statusKey, i) => {
      const eventName = STATUS_MAP[statusKey] || 'EI_Event';
    ...
    <div className="space-y-6 animate-fadeIn h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0 shrink-0">
        ...
        <div className="flex items-center space-x-3 self-start sm:self-auto">
          ...
          <span className="bg-slate-100 border border-slate-200 text-slate-600 px-3 py-1.5 rounded-xl text-xs font-bold shadow-sm">
            Read-Only Canvas
          </span>
        </div>
      </div>
